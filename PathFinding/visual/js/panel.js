...
var Panel = {
    init: function() {
		...
		$('#btnCriar').click(function(){
			...
			Controller.none();
			$('#draw_area').children().remove()
			Controller.gridSize = [w,h];
			View.startNode=null;
			View.endNode =null;
			Controller.init();
		...
		$('#btnExp').click(function(){
			var text = 
				Controller.grid.nodes.map(function(e, x) {
					return e.map(function(e2, y) {
						if (x == Controller.startX && y == Controller.startY) {
							return "S";
						}
						if (x == Controller.endY && y == Controller.endX) {
							return "E";
						}
						return (e2.walkable ? "-" : "#")
					}).join('')
